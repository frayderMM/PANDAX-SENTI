    ...
    <main class="contenido">
      ...
      <template v-if="tablero">
        ...
        <section class="grid">
          ...
          <article class="tarjeta zonas"><div class="tarjeta-cabeza"><div><h2>Zonas prioritarias</h2><p>Agregación de las últimas 24 horas</p></div><span class="etiqueta">Sin puntos individuales</span></div><p v-if="!zonas.length" class="vacio">No hay reportes suficientes para mostrar zonas prioritarias.</p><ul v-else><li v-for="(zona, indice) in zonas" :key="indice"><span class="zona-numero">{{ indice + 1 }}</span><span>Celda aproximada {{ zona.geometry.coordinates[1].toFixed(3) }}, {{ zona.geometry.coordinates[0].toFixed(3) }}</span><b>{{ zona.properties.reportes }} reportes</b></li></ul></article>
        </section>
