...
`POST /municipal/alertas` es quien de verdad usa esa lista: al publicar,
cuenta los suscriptores activos del distrito de la alerta y, si hay alguno,
manda la tarea `enviar_alerta_whatsapp` (`app/tasks/celery_app.py`) a la cola.
La tarea no decide nada —ni el texto, ni a quién, ni si se manda—: solo
entrega lo que el operador ya confirmó. Si `SENTI_WHATSAPP_ENABLED=false`,
la alerta igual se publica (§22 no depende de un canal externo) y la
respuesta lo dice en `notificacion_whatsapp`, en vez de fallar todo el
endpoint por un canal que no es su responsabilidad.
